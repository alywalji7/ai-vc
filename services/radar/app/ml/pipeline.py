...
from ..database import (
    Company, InvestmentOpportunity, CompanyOutcome, ModelVersion,
    get_db, SessionLocal
)
...
def prepare_training_data(db: Session) -> Tuple[pd.DataFrame, np.ndarray]:
    """
...
def run_training_pipeline():
    ...
    try:
        with SessionLocal() as db:
            ...
            setup_mlflow()
            with mlflow.start_run() as run:
                model, metadata = train_model(X, y)
