...
def rate_limit(
    ...
    """
    def decorator(func):
        # Apply token limiting if needed (imported here to avoid circular imports)
        if token_quota is not None:
            try:
                from .openai_metering import token_rate_limit
                func = token_rate_limit(user_quota=token_quota)(func)
            except ImportError:
                logger.warning("OpenAI token metering not available, token quota disabled")
        
        # Apply GPU limiting if needed
        try:
            from .gpu_metering import gpu_rate_limit, GPU_ENABLED
            ...
            logger.warning("GPU metering not available, GPU quota disabled")
