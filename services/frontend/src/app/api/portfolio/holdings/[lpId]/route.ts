...
/**
 * GET handler for retrieving portfolio holdings for an LP
...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(`${UPLOAD_SERVICE_URL}/api/v1/holdings/${lpId}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.detail || 'Failed to fetch portfolio holdings' },
